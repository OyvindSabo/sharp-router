...
const getTokens = (hash: Hash) => hash
    .split('/')
    .filter(Boolean)
...
const parseParameterRouteToken = (token: Token) => {
  ...
  hashTokens.map((hashToken, i) => [hashToken, routeTokens[i]]).forEach(([hashToken, routeToken]) => {
    if (isParameterRouteToken(routeToken)) {
      const { parameter, type } = parseParameterRouteToken(
        routeToken
      );
      if (type === 'int') {
        if (!isInt(hashToken)) return false;
        params[parameter] = parseInt(hashToken);
        reconstructedHash = appendTokenToHash(
          reconstructedHash,
          hashToken
        );
